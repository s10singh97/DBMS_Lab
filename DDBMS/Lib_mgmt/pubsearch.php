...
{
  $pname=addslashes($_POST['pubname']);
$conn=new mysqli($hn,$un,$pw,$db);
if($conn->connect_error) die($conn->connect_error);
$query="select * from publisher where name like '%".$pname."%' ";
$result=$conn->query($query);
$rows=$result->num_rows;
if($rows>0)
{
  $q="select p.id as pubid,name,printing_house,(select count(*) from books where publisher_id=pubid) as no_published from publisher as p where p.name like '%".$pname."%' ";
echo<<<_end
<!DOCTYPE html>
<html>
<head>
...
background-color: black" >
  ...
  <div>
  <span style="text-decoration:underline; margin-left: 38%;font-size: 40px;font-weight: 600">PUBLISHER DETAILS</span>
</div>
